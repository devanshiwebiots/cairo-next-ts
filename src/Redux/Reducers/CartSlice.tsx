import { CartSliceProp } from "@/Types/EcommerceType";
...
const CartSlice = createSlice({
  ...
  reducers: {
    ...
    decrementData: (state, action) => {
      const productId = action.payload.item.id;
      state.cart = state.cart.map((item) => 
        productId === item.id && item.total > 1 
          ? { 
              ...item, 
              total: item.total - 1, 
              sum: item.price * (item.total - 1) 
      ...
      if (existingProductIndex !== -1) {
        state.cart = state.cart.map((product, index) =>
          index === existingProductIndex
            ? {
                ...product,
                total: product.total + action.payload.quantity,
                sum: product.price * (product.total + action.payload.quantity),
              }
            : product
        );
      }  else {
        state.cart.push({
          ...
          sum: action.payload.item.price * action.payload.quantity,
        });
      }
    }
  },
