            ...
            <Col md='10'>
                <Range values={values} step={STEP} min={MIN} max={MAX} onChange={(values) => {setValues(values)}} renderTrack={({props, children}) => (
                    <div onMouseDown={props.onMouseDown} onTouchStart={props.onTouchStart} style={{...props.style, height:'36px'}} className='d-flex w-100 mt-2 mb-3'>
                        ...
                        <div ref={props.ref} className='w-100' style={{height:'15px', borderRadius:'4px', background:getTrackBackground({values, colors: ['#ccc', '#61AE41','#ccc'], min: MIN, max:MAX}), alignSelf: 'center'}}>{children}</div>
                        <output className="mt-2 ms-2">{values[1]}</output>
                    </div>
                )}
                renderThumb = {({props,index}) => (
                    <div {...props} key={index} className='d-flex justify-content-center align-items-center' style={{...props.style, height:'25px', width:'7px', background:'#61AE41', boxShadow:'0px 2px 6px #AAA'}}>
                        <div style={{height:'16px', width:'5px', background:"#61AE41"}}></div>
                    </div>
                )}
                />
            </Col>
