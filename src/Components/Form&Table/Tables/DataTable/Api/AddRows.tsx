    ...
    <Col sm="12">
      <Card className="basic-data-table">
        <CardHeader className="pb-0 card-no-border">
          <h4 className="mb-2">Add rows </h4>
          <span>
            New rows can be added to a DataTable using the
            <code className="api" title="DataTables API method">
              row.add()
            </code>
            API method. Simply call the API function with the data for the new row (be it an array or object). Multiple rows can be added using the
            <code className="api" title="DataTables API method">
              rows.add()
            </code>
        ...
        <CardBody>
          <Button color="primary" tag="a" className="mb-3" onClick={addNewRow}>
            {AddNewRowButton}
          </Button>
          <div className="table-responsive">
            <div id="API-1" className="dataTables_wrapper">
              <DataTable data={filteredItems} columns={AddRowColumn} striped={true} className="display theme-scrollbar" pagination subHeader subHeaderComponent={subHeaderComponentMemo} />
            </div>
          </div>
        </CardBody>
      </Card>
