...
const DataLoopOne = () => {
  const { bookmarkOne } = useAppSelector((state) => state.bookmarkTab);
  const dispatch = useAppDispatch();

  const addToFavorites = (data: BookMarkData) => {
    if (data.fillStar === false) {
      const newBookMarkData = bookmarkOne.map((item) => (item.id === data.id ? { ...item, fillStar: true } : item));
      dispatch(setBookmark(newBookMarkData));
    } else {
      const newBookMarkData = bookmarkOne.map((item) => (item.id === data.id ? { ...item, fillStar: false } : item));
      dispatch(setBookmark(newBookMarkData));
    }
  };
  ...
  return (
    <Row id="bookmarkData">
      {bookmarkOne.length > 0 ? (
        bookmarkOne.map((data, index) => (
          <Col xxl="3" md="4" sm="6" key={index} className="col-ed-4">
            <Card className="card-with-border bookmark-card o-hidden">
              <div className="details-website">
                <RatioImage className="img-fluid" src={`${ImagePath}/${data.image}`} alt="image" />
