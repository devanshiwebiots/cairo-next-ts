...
const ContactDetailsClass: React.FC<ContactDetailsPropsType> = ({selectedUser, setSelectedUser, userEditCallback}) => {
    ...
    const editUsers = (userData: UserCallbackUser) => {
        dispatch(setTempId(userData.id))
        userEditCallback(true, userData)
    }
       ...
       {selectedUser ? (
        <div className="profile-mail">
          <div className="d-flex">
            <RatioImage className="img-100 img-fluid m-r-20 rounded-circle update_img_0" src={`${selectedUser.avatar}`} alt="" />
