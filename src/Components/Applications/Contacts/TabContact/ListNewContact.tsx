...
const ListNewContact: React.FC<ListNewContactPropsType> = ({ userCallback, users }) => {
            ...
            return (
              <NavLink className={dynamicTab === index ? "active" : ""} onClick={() => setDynamicTab(index)} key={index}>
                <div className="d-flex" onClick={() => ContactDetails(user)}>
                  <RatioImage className="p-0 img-fluid img-50 m-r-20 rounded-circle update_img_0" src={`${user.avatar}`} alt="userImage" />
