...
const InboxEmailContent :React.FC<CommonDataType> = ({data,ids}) => {
  ...
  const handleInterviewValue = () => dispatch(handleInterview(true));
  ...
  const handleRemoveEmail = (id: number) => {
    ...
    return (
      ...
      <div className="inbox-message">
        <div className="email-data" onClick={handleInterviewValue}>
          <span>
            {data.message}
          ...
          <div className="inbox-width d-flex gap-2">
            {data.badge &&
              data.badge.map((item, i) => (
                <Badge
                  color=""
                  className={`m-0 badge-width badge-light-${item.color} text-${
                    item.color === "light" ? "light-dark" : item.color
                  }`}
                  key={i}
                >
                  {item.title}
        ...
        <div className="email-options">
          <i
            className={`fa fa-envelope-o envelope-1 ${!faIcon ? "show" : "hide"}`}
            onClick={() => dispatch(handleEnvelope(true))}
          />
          <i
            className={`fa fa-envelope-open-o envelope-2 ${faIcon ? "show" : "hide"}`}
